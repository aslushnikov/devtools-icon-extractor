...
function lineAt(line, dir) {
  ...
  var pos = dir == -1 ? order.length - 1 : 0
  return (dir == -1 ? bidiRight : bidiLeft)(order[pos])
}

function lineLeft(line) { return lineAt(line, 1) }
function lineRight(line) { return lineAt(line, -1) }
...
function moveLogically(line, start, dir) {
  var target = moveInLine(line, start.ch, dir)
...
function endOfLine(visually, cm, lineObj, lineNo, dir) {
  ...
  if (visually) {
    ...
    if (order) {
      var i = dir < 0 ? order.length - 1 : 0
      var part = order[i]
      var sticky$1 = (dir < 0) != (part.level == 1) ? "before" : "after"
      ...
      if (part.level > 0) {
        ...
        if (sticky$1 == "before") { ch = moveLogically(lineObj, new Pos(lineNo, ch, sticky$1), 1, true) }
      } else { ch = (dir < 0 ? bidiRight : bidiLeft)(part) }
      return new Pos(lineNo, ch, sticky$1)
    }
  }
...
function getVisualLineAround(cm, line, target) {
  ...
  var prep = prepareMeasureForLine(cm, line)
  var targetTop = measureCharPrepared(cm, prep, target).top
  return [
    findFirst(function (ch) { return targetTop == measureCharPrepared(cm, prep, ch).top; }, 0, target),
    findFirst(function (ch) { return targetTop == measureCharPrepared(cm, prep, ch).top; }, line.text.length - 1, target)
  ]
}

function moveVisually(cm, line, start, dir) {
  var mkPos = function (ch, sticky) { return ch == null ? null : new Pos(start.line, ch, sticky); }
  var mv = function (pos, dir) { return moveLogically(line, pos instanceof Pos ? pos : new Pos(start.line, pos), dir); }
  ...
  var getVisualLine = function (ch) { return getVisualLineAround(cm, line, ch); }
  var visualLine = getVisualLine(start.sticky == "before" ? mv(start, -1) : start.ch)
  ...
  var searchInVisualLine = function (partPos, dir, visualLine) {
    ...
    for (; partPos >= 0 && partPos < bidi.length; partPos += dir) {
      ...
      var ch = moveInStorageOrder ? visualLine[0] : visualLine[1]
  ...
  var res = searchInVisualLine(partPos + dir, dir, visualLine)
  if (res) { return res }

  // Case 3b: Look for other bidi parts on the next visual line
  var nextCh = mv(visualLine[dir > 0 ? 1 : 0], dir)
...
function wrappedLineExtent(cm, lineObj, preparedMeasure, y) {
  ...
  var begin = findFirst(function (ch) { return measure(ch).bottom < y; }, end - 1, 0) + 1
...
function findPosH(doc, pos, dir, unit, visually) {
  ...
  function moveOnce(boundToLine) {
    ...
    if (visually) {
      ...
      var ch = moveLogically(lineObj, pos, dir)
